...
\[
\min \sum_{p=1}^{\text{num_parts}} \sum_{q=1}^{\text{num_parts}} \text{color_cost}[ \text{parts_colors}[p]] \times \text{color_change}[p, q]
\]
...
6. **No Overlap**:
   - If part \( p \) is scheduled before part \( p' \), then \( \text{end_times}[p] \leq \text{start_times}[p'] \).
